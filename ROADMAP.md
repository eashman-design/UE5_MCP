...
- [x] Create repo root directory structure matching DESIGN.md §3 (plugin/, mcp_server/, docs/, .github/)
- [x] Add .gitignore covering UE5 build artifacts (Binaries/, Intermediate/, .vs/, __pycache__, *.pyc, dist/, .venv/)
- [x] Add MIT LICENSE
- [x] Add stub README.md with project overview, install steps, and Claude Code config block

### Milestone 1.2 — Plugin Skeleton
- [x] Create `plugin/UE5MCP/UE5MCP.uplugin` per DESIGN.md §4.2 (Editor type, Default loading phase)
- [x] Create `UE5MCP.Build.cs` with Core/CoreUObject/Engine/Slate/Json/DeveloperSettings in unconditional block; UnrealEd/LevelEditor/HTTPServer/HTTP inside `if (Target.bBuildEditor)` guard
- [x] Implement `FUE5MCPModule` (StartupModule/ShutdownModule log only — no real init) in `UE5MCPModule.h/.cpp`
- [x] Add `IMPLEMENT_MODULE` and `DEFINE_LOG_CATEGORY_STATIC(LogUE5MCP, Log, All)` in module .cpp
...
- [x] Implement `UUE5MCPSettings : UDeveloperSettings` with `Port` (int32, default 8765) and `bEnableOnStartup` (bool, default true) UPROPERTY fields
...
- [x] Implement `UUE5MCPSubsystem : UEditorSubsystem` with Initialize/Deinitialize, StartServer/StopServer/IsServerRunning
- [x] Wire Initialize to read `bEnableOnStartup` and conditionally call StartServer
- [x] Wire Deinitialize to call StopServer then tear down LogCapture
...
- [x] Implement `FUE5MCPHttpServer` with Start/Stop/IsRunning and `RegisterAllRoutes` dispatcher
- [x] Implement `UE5MCPRouteRegistrar.h` — define the `RegisterRoutes(TSharedPtr<IHttpRouter>)` function signature contract all handler groups must satisfy
- [x] Wire `FUE5MCPHttpServer::Start` to `FHttpServerModule::Get().GetHttpRouter(Port)` and `StartAllListeners()`; Stop to `StopAllListeners()` and Router.Reset()
- [x] Add startup/shutdown log lines confirming port number
...
- [x] Implement `FUE5MCPResponseHelpers` header-only struct with `BuildSuccessResponse(TSharedPtr<FJsonObject>)` and `BuildErrorResponse(FString Message, FString Code)`
- [x] Both helpers produce `{"ok": true/false, ...}` envelope, always HTTP 200, Content-Type application/json
...
- [x] Implement `FUE5MCPLogCapture : FOutputDevice` with `StartCapture` (GLog->AddOutputDevice), `StopCapture` (GLog->RemoveOutputDevice), and circular buffer of 2000 lines
- [x] `Serialize` method acquires `FCriticalSection Lock` before touching `Buffer` — called from arbitrary threads
- [x] `GetRecentLines(int32 Count, FString Category)` acquires Lock, returns tail slice optionally filtered by category substring
- [x] Integrate into subsystem: create LogCapture in Initialize before StartServer, StopCapture and Reset in Deinitialize
...
- [x] Create `mcp_server/` directory tree per DESIGN.md §3 (src/ue5_mcp/, tests/, pyproject.toml, .python-version)
- [x] Write `pyproject.toml` per DESIGN.md §6.1 — hatchling backend, mcp>=1.0.0, httpx>=0.27.0, pydantic>=2.0.0, entry point `ue5-mcp = "ue5_mcp.__main__:main"`
- [x] Write `.python-version` containing `3.11`
- [ ] Confirm `pip install -e .` succeeds and `ue5-mcp --help` prints usage

### Milestone 4.2 — Config and Entry Point
- [x] Implement `Config` dataclass per DESIGN.md §6.3 with editor_port, editor_host, request_timeout, log_level fields and `from_args()` classmethod
- [x] Implement `__main__.py` — parse Config, create server, run stdio loop
- [x] Implement `__init__.py` with package version string

### Milestone 4.3 — Plugin HTTP Client
- [x] Implement `EditorClient` per DESIGN.md §6.4 — wraps httpx.AsyncClient, `get()` and `post()` methods, `_unwrap()` that raises `EditorError` on `ok=false` and `EditorConnectionError` on connection failure
- [x] `EditorConnectionError` message is human-readable for Claude (per DESIGN.md §6.7)
- [x] `aclose()` for clean shutdown
...
- [x] Implement `server.py` — `create_server(config)` creates MCP Server instance, `list_tools()` aggregates TOOLS from all modules, `call_tool()` dispatches to correct module handle() by checking TOOL_NAMES sets
- [x] Raises `ValueError` for unknown tool names
- [ ] Confirm server starts, connects to running editor, and `list_tools` returns all 13 tool definitions

### Milestone 4.5 — Actor Tools
- [x] Implement `tools/actors.py` per DESIGN.md §6.6 canonical pattern: SpawnActorInput/DeleteActorInput Pydantic models, TOOLS list with 3 Tool definitions, TOOL_NAMES set, handle() function
- [x] Tool descriptions written for Claude's benefit — describe what the tool does and what it returns
...
- [x] Implement `tools/transforms.py`: GetTransformInput/SetTransformInput models (name required; location/rotation/scale Optional[list[float]])
- [x] TOOLS: `get_actor_transform`, `set_actor_transform`
- [ ] Unit tests: success round-trip, partial set (location only), ACTOR_NOT_FOUND propagation

### Milestone 4.7 — Property Tools
- [x] Implement `tools/properties.py`: GetPropertyInput/SetPropertyInput models; all values as strings per protocol
- [x] TOOLS: `get_property`, `set_property`; descriptions explain string format for FVector, FRotator, FLinearColor
...
- [x] Implement `tools/console.py`: ExecuteConsoleInput, `execute_console_command` tool
- [x] Implement `tools/pie.py`: StartPIEInput (mode field with enum: selected_viewport/simulate/new_window), `start_pie`/`stop_pie`/`get_pie_state` tools
- [x] Implement `tools/screenshot.py`: CaptureScreenshotInput (optional filename), `capture_screenshot` tool
- [x] Implement `tools/logs.py`: GetLogInput (optional count int, optional category str), `get_log` tool
...
- [x] Add `.github/workflows/python-tests.yml` — runs `pytest` on Python 3.11 on ubuntu-latest; triggers on push and PR to main
- [x] Add `.github/workflows/python-lint.yml` — runs `ruff check` and `mypy --strict` on the mcp_server/src tree
